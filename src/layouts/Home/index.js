...
export default ({ headingText = "Capsules " }) => {
  let history = useHistory();
  const [visible, setVisible] = useState(7);
  const [entries, setEntries] = useState([]);
  useEffect(() => {
    getCategories(setEntries);
  }, []);
    ...
    <StyledDiv className="bg-gray-200">
      ...
      <Container>
        <ContentWithPaddingXl>
          ...
          {isMobile ? (
            <Posts>
              {entries.slice(0, visible).map((cat, index) => (
                <PostContainer
                  key={cat.id}
                  // featured={post.featured}
                  onClick={() =>
                    history.push({
                      pathname: `main/${cat.id}`,
                    })
                  }
                >
                  <Post
                  ...
                  >
                    ...
                    <Info>
                      ...
                      <CreationDate>{cat.noteCount} notes</CreationDate>
                ...
                >
                  ...
                  >
                    ...
                    <FeatInfo className=" rounded-r-lg ">
                      ...
                      <FeatDescription>{cat.description}</FeatDescription>
                    </FeatInfo>
                  </FeatPost>
                </FeatPostContainer>
              ))}
